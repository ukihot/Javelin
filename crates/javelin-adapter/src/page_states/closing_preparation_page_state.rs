// ClosingPreparationPageState - PageState implementation for closing preparation history screen
...
impl ClosingPreparationPageState {
    pub fn new(controllers: &Controllers) -> Self {
        ...
        let mut page = ClosingPreparationPage::new();
        page.set_loading();

        let controller = std::sync::Arc::clone(&controllers.batch_history);
        let batch_type = "ClosingPreparation".to_string();
        ...
        loop {
                    ...
                    event::read().map_err(crate::error::AdapterError::EventReadFailed)?
            {
                ...
                match key.code {
                    ...
                    KeyCode::Char('e') => {
                        return Ok(NavAction::Go(Route::ClosingPreparationExecution));
                    }
...
impl Drop for ClosingPreparationPageState {
    fn drop(&mut self) {
