// SubsidiaryAccountMasterPageState - 補助科目マスタ画面の状態
...
use crate::{
    ...
    presenter::{SubsidiaryAccountMasterPresenter, SubsidiaryAccountMasterViewModel},
    views::pages::SubsidiaryAccountMasterPage,
};

/// 補助科目マスタ画面の状態
pub struct SubsidiaryAccountMasterPageState {
    /// Unique identifier for presenter registration
    id: Uuid,
    /// Reference to presenter registry
    registry: Arc<PresenterRegistry>,
    /// The page view
    page: SubsidiaryAccountMasterPage,
    /// Data receiver channel
    data_rx: tokio::sync::mpsc::UnboundedReceiver<SubsidiaryAccountMasterViewModel>,
    /// 現在のページ番号（0始まり）
    current_page: usize,
    /// 1ページあたりの表示件数
...
impl SubsidiaryAccountMasterPageState {
    pub fn new(registry: Arc<PresenterRegistry>) -> Self {
        let id = Uuid::new_v4();

        // Create channel and presenter
        let (tx, rx) = SubsidiaryAccountMasterPresenter::create_channel();
        let presenter = Arc::new(SubsidiaryAccountMasterPresenter::new(tx));

        // Register presenter
        registry.register_subsidiary_account_master_presenter(id, presenter);

        Self {
            id,
            registry,
            page: SubsidiaryAccountMasterPage::new(),
            data_rx: rx,
            current_page: 0,
            items_per_page: 10,
            selected_index: 0,
...
impl PageState for SubsidiaryAccountMasterPageState {
    ...
    fn run(
        ...
        if !self.data_loaded {
            ...
            let controller = Arc::clone(&controllers.subsidiary_account_master);
            let page_id = self.id;
...
impl Drop for SubsidiaryAccountMasterPageState {
    fn drop(&mut self) {
        // Unregister presenter when page is destroyed
        self.registry.unregister_subsidiary_account_master_presenter(self.id);
    }
}
