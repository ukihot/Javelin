...
/// 売掛金明細項目ViewModel
#[derive(Debug, Clone)]
pub struct ArDetailItemViewModel {
    pub date: String,
    pub customer: String,
...
impl MasterListItem for ArDetailItemViewModel {
    fn headers() -> Vec<&'static str> {
        vec!["日付", "取引先", "請求番号", "摘要", "金額", "入金状況", "期日"]
    }
...
impl ArDetailPageState {
    ...
    fn poll_ledger_data(&mut self) {
        while let Ok(ledger_data) = self.ledger_rx.try_recv() {
            let items: Vec<ArDetailItemViewModel> = ledger_data
                .entries
                .into_iter()
                .map(|entry| ArDetailItemViewModel {
...
impl Drop for ArDetailPageState {
    fn drop(&mut self) {
        // TODO: PresenterRegistryから登録解除
    }
}

impl PageState for ArDetailPageState {
    fn route(&self) -> Route {
        Route::ArDetail
    }
