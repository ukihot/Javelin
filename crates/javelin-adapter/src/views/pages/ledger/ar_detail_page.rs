...
impl ArDetailPage {
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        ...
        let title = Paragraph::new("B-06: AR Detail (売掛金明細)")
        ...
        if let Some(ref data) = self.ledger_data {
            ...
            let rows: Vec<Row> = data
                ...
                .map(|(i, entry)| {
                    ...
                    let amount = entry.debit_amount - entry.credit_amount;
                    Row::new(vec![
            ...
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(format!("売掛金明細 ({}件)", data.entries.len())),
            );
