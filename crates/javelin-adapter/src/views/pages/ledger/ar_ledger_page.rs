...
impl ArLedgerPage {
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        ...
        let title = Paragraph::new("B-05: AR Subsidiary Ledger (売掛金補助元帳)")
            .block(Block::default().borders(Borders::ALL))
            .style(Style::default().fg(Color::Cyan));
        frame.render_widget(title, chunks[0]);

        if let Some(ref data) = self.ledger_data {
            let header = Row::new(vec!["日付", "取引先", "摘要", "借方", "貸方", "残高"])
            ...
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(format!("売掛金補助元帳 ({}件)", data.entries.len())),
            );
